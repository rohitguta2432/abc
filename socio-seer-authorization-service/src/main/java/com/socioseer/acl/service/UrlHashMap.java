 ...
 *
 */
public class UrlHashMap<T> implements Map<String, T> {
	...
	/**
	 * checks whether the cache or container contain a specific key, then
	 * evaluates the container's keys as regexes and checks whether they match
	 * the specific key.
	 */
	public boolean containsKey(Object key) {
		...
		// check if the requested key is a matching string of a regex key from
		// our container
	...
	/**
	 * checks whether a specific value is container within either container or
	 * cache
	 */
	...
	/**
	 * returns a merged entryset containing within both the container and cache
	 * entrysets
	 */
	...
	/**
	 * checks whether the requested key has a direct match in either cache or
	 * container, and if it doesn't, also evaluates the container's keyset as
	 * regexes to match against the input key and if any of those methods yield
	 * a value, returns that value if a value is found doing regex evaluation,
	 * use that regex-key's match as a non-regex key with the regex's value to
	 * form a new entry in the cache.
	 */
	public T get(Object key) {
		...
		if ((result = cache.get(key)) != null) {
			...
			// check if the requested key is a matching string of a regex key
			// from our container
		...
		// no value for the given key was found in any of
		// container/cache/regexkey-container
		return null;
	}
	 ...
	 * @param value
	 *            Value to map to
	 * @return previous value associated with the key, or null if unassociated
	 *         before
	 */
	...
	/**
	 * returns the combined collection of both the values of the container as
	 * well as the cache.
	 */
